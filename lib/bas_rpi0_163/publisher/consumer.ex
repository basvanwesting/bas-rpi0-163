defmodule BasRpi0163.Publisher.Consumer do
  use GenStage
  require Logger
  use AMQP
  ...
  def handle_info(:connect, state) do
    with {:ok, queue} <- Application.fetch_env(:amqp, :sensor_measurements_queue),
         {:ok, conn} <- BasRpi0163.Publisher.AMQP.get_connection(),
         {:ok, chan} <- Channel.open(conn),
         :ok <- GenStage.async_subscribe(__MODULE__, to: BasRpi0163.Publisher.Producer, max_demand: 1) do
